function [estate_matref,istate_matref] = compute_IVrefMM(res,params,estate_mat_before,istate_mat_before)
if nargin > 4
    error('输入变量过多！');
elseif nargin == 2
    flag = 0; % 默认情况下无before
...
if isfield(res, 'iref')
    ...
    ref_e(ref_e>1)=1;
    ref_i(ref_i>1)=1;
    ref_e = ceil(ref_e/refbin);
    ref_i = ceil(ref_i/refbin);
else
    tau_ref = 3;
    ht = 0.1:0.1:10;
    ht2 = 0:0.1:9.9;
...
ref_e_statenum = max_ref_e-min_ref_e+1;
ref_i_statenum = max_ref_i-min_ref_i+1;
...
%% 合并同类项（只统计V就不需要这一步了）
if flag
    estate_matref = estate_mat_before;
else
    estate_matref = cell(V_e_statenum,I_e_noref_statenum, ref_e_statenum);
end
if flag
    istate_matref = istate_mat_before;
else
    istate_matref = cell(V_i_statenum,I_i_noref_statenum, ref_i_statenum);
end
% 同时统计Vi+1与Nfi+1
for i = 300:length(V_e)-1
    estate_matref{V_e(i)-min_V_e+1,I_e_noref(i)-min_I_e_noref+1, ref_e(i)-min_ref_e+1} = ...
        [estate_matref{V_e(i)-min_V_e+1,I_e_noref(i)-min_I_e_noref+1, ref_e(i)-min_ref_e+1};[V_e(i+1) nfe(i+1) 1]];
end
% 同时统计Vi+1与Nfi+1
for i = 300:length(V_e)-1
    istate_matref{V_i(i)-min_V_i+1,I_i_noref(i)-min_I_i_noref+1, ref_i(i)-min_ref_i+1} = ...
        [istate_matref{V_i(i)-min_V_i+1,I_i_noref(i)-min_I_i_noref+1, ref_i(i)-min_ref_i+1};[V_i(i+1) nfi(i+1) 1]];
end

for i = 1:V_e_statenum*I_e_noref_statenum*ref_e_statenum
    estate = estate_matref{i};
    if ~isempty(estate)
        ...
        estate_matref{i} = temp;
    end
end

for i = 1:V_i_statenum*I_i_noref_statenum*ref_e_statenum
    istate = istate_matref{i};
    if ~isempty(istate)
